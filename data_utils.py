...
def au2heatmap(img_path, label, size, sigma, args):
	...
	if args['dataset'] == 'BP4D':
		...
		# au10 lmk 50, 52
		heatmap = add_noise(heatmap, 4, label, lmk[50], lmk[52], sigma, size)
		...
		# au23 lmk 51, 57 / lip center
		heatmap = add_noise(heatmap, 8, label, lmk[51], lmk[57], sigma, size)

		# au24 lmk 51, 57 / lip center
		heatmap = add_noise(heatmap, 9, label, lmk[51], lmk[57], sigma, size)
