...
class pixel_interpreter(nn.Module):
	def __init__(self, points, dim):
		super(pixel_interpreter, self).__init__()
...
class cnn_interpreter(nn.Module):
	def __init__(self, points, dim, dropout=0.1):
		super(cnn_interpreter, self).__init__()
		self.layers = nn.Sequential(
			nn.Conv2d(dim, 512, kernel_size=5, padding=2),
			nn.ReLU(),
